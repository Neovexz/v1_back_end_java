...
public class ChamadoController {
    ...
    public ResponseEntity<ChamadoResponse> criar(@Valid @RequestBody ChamadoCreateDto dto) {
        var resp = chamadoService.criar(dto);
    ...
    public ResponseEntity<Page<ChamadoResponse>> listar(Pageable pageable) {
        return ResponseEntity.ok(chamadoService.listar(pageable));
    }

    @PatchMapping("/{id}/status")
    public ResponseEntity<ChamadoResponse> atualizarStatus(@PathVariable Long id, @RequestBody String status) {
        var s = com.cortexia.cortexia_back_end.enums.StatusChamado.valueOf(status.toUpperCase());
        return ResponseEntity.ok(chamadoService.atualizarStatus(id, s));
    }
}
